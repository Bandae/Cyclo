...
def oblicz_Mgmax(wariant_podparcia, F_j, E, b_kola):
    ...
    if wariant_podparcia == 1:
        ...
        M_gI = -(F_j/2 * l_1) + F_j/2 * l_1
        M_gII = -(F_j/2 * l_k2) + F_j/2 * l_k2
        ...
        M_gI = -M_uI + (F_j/2 - (F_j/2 * ((l_1 ** 3) / (2 * (l_k3 ** 3))) * (3 * l_k3 - l_1))) * l_1
        M_gII = -M_uII + (F_j/2 - (F_j/2 * ((l_k2 ** 3) / (2 * (l_k3 ** 3))) * (3 * l_k3 - l_k2))) * l_k2
        M_gmax = max(M_gmaxI, M_gmaxII)

    elif wariant_podparcia == 6:
        R_BI = F_j/2 * ((2 * ((l_k3 - l_1) ** 3) - 3 * ((l_k3 - l_1) ** 2) * l_k3+ (l_k3 ** 3)) / (l_k3 ** 3))
        R_BII = F_j/2 * ((2 * ((l_k3 - l_k2) ** 3) - 3 * ((l_k3 - l_k2) ** 2) * l_k3 + (l_k3 ** 3)) / (l_k3 ** 3))

        M_uAI = F_j/2 * ((((l_k3 - l_1) ** 2) * l_k3) / (l_k3 ** 2))
        M_uAII = F_j/2 * ((((l_k3 - l_k2) ** 2) * l_k2) / (l_k3 ** 2))
        M_uBI = F_j/2 * (((l_k3 - l_1) * (l_1 ** 2)) / (l_k3 ** 2))
        M_uBII = F_j/2 * (((l_k3 - l_k2) * (l_k2 ** 2)) / (l_k3 ** 2))
...
def oblicz_sily(M_k, R_wt, n_sworzni):
    F_list = []
    ...
    for i in range(1, n_sworzni + 1):
        fi_kj = obl_fi_kj(i)
        F_j = M_k * (R_wt * math.sin(fi_kj) / sum(
            [(R_wt ** 2) * (math.sin(obl_fi_kj(j)) ** 2) for j in range(1, n_sworzni + 1)]
        ))
        F_list.append(F_j)
    
    return F_list

def obliczenia_mech_wyjsciowy(wariant_podparcia, k_g, E, b_kola, n_sworzni, M_k, R_wt):
    '''
    E, k_g, b_kola, R_wt - wziac z interfejsu od uzytkownika
    M_k - policzone, od Pawla
    '''
    sily = oblicz_sily(M_k, R_wt, n_sworzni)
